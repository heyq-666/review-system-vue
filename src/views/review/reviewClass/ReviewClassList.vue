<template>
  <div>
          ...
          <a-dropdown v-if="selectedRowKeys.length > 0">
              <template #overlay>
                <a-menu>
                  ...
                  <a-menu-item key="2" @click="batchPublish(0)">
                    <Icon icon="ant-design:lock-outlined"></Icon>
                    停用
                  </a-menu-item>
                  <a-menu-item key="3" @click="batchPublish(1)">
   ...
   <!--子表表格tab-->
    ...
    <!-- 表单区域 -->
    <ReviewClassModal @register="registerModal" @success="handleSuccess"></ReviewClassModal>
  </div>
</template>
...
<script lang="ts" name="reviewClass-reviewClass" setup>
  ...
  function batchPublish(status) {
    createConfirm({
      iconType: 'warning',
      title: '确认操作',
      content: '是否' + (status == 1 ? '发布' : '停用') + '选中量表?',
      onOk: async () => {
        await publishBatch({ ids: unref(selectedRowKeys).join(','), status: status }, reload);
      },
    });
  }
